# Changelog

## Unreleased

- Added platform-ready runtime bundle for `volcano`.
- Added platform-ready runtime bundles for `pathway_l2p_single` and `pathway_l2p_multi`.
- Added guarded sync documentation and tooling for isolated L2P runtime development.

## 0.0.1

- Created initial OMIX_Test monorepo scaffold.
- Added shared starter environment folders.
- Added independent module folders and manifests.
- Added repo-level contract test and CI workflow placeholders.
