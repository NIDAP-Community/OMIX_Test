...
module_name <- "pathway_l2p_multi"
display_name <- "OMIX L2P Multi"
source_template <- "L2P_Multi_v93.R"
entry_function <- "l2p_multi"
...
function_args$export_plot_file <- file.path(results_dir, "l2p_multi_plot.png")
function_args$export_results_file <- file.path(results_dir, "l2p_multi_results.csv")

required_columns <- unique(c(function_args$gene_names_column, unlist(function_args$t_statistic_columns), unlist(function_args$significance_columns), unlist(function_args$fold_change_columns)))
...
if (opts$dry_run) {
  jsonlite::write_json(
    ...
    file.path(results_dir, "pathway_l2p_multi_dry_run.json"),
    pretty = TRUE,
    auto_unbox = TRUE
  )
