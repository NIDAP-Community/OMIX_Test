...
Independent OMIX module for L2P multi-comparison pathway analysis.
...
Open the repository in Positron or another R-capable editor. For notebook workflows, use an R Markdown or Quarto document and keep the working directory at `modules/pathway_l2p_multi/runtime`.

Install or verify the R packages for the runtime once before running interactively:

```bash
cd modules/pathway_l2p_multi/runtime
...
```text
runtime/code/functions/L2P_Multi_v93.R
```

Set the working directory to `modules/pathway_l2p_multi/runtime` before running or sourcing runtime files interactively.
